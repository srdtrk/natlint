...
impl Comment {
    ...
    pub fn from_doc_comment(value: DocCommentTag) -> Option<Self> {
        CommentTag::from_str(&value.tag).map(|tag| Self {
            tag,
            value: value.value,
        })
    }
...
impl From<Vec<DocCommentTag>> for Comments {
    fn from(value: Vec<DocCommentTag>) -> Self {
        Self(
            value
                .into_iter()
                .flat_map(Comment::from_doc_comment)
                .collect(),
        )
    }
}
...
mod tests {
    ...
    fn parse_comment_tag() {
        ...
        assert_eq!(
            CommentTag::from_str("inheritdoc"),
            Some(CommentTag::Inheritdoc)
        );
        assert_eq!(
            CommentTag::from_str("custom:"),
            Some(CommentTag::Custom(String::new()))
        );
        assert_eq!(
    ...
    fn test_is_custom() {
        ...
        assert!(
            custom_comment.is_custom(),
            "Custom tag should return true for is_custom"
        );
